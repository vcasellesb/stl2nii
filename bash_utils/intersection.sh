...
if $reverse; then
    seg_maths ${new_object} -mul "$working_dir"tmp_sum/intersection-inv.nii.gz -add ${total_accumulated} ${total_accumulated}
else
    seg_maths ${total_accumulated} -mul "$working_dir"tmp_sum/intersection-inv.nii.gz -add ${new_object} ${total_accumulated}
fi
